...
let selectedReturnTripDate = '';
let availableReturnDatesSet = new Set();
let returnDatePickerInstance = null;
let tripMode = 'one-way';
let selectedClassFilter = 'economy';
...
const DOM = {
    ...
    get classFilterBar() { return document.getElementById('classFilterBar'); },
    get classFilterButtons() { return document.querySelectorAll('.class-filter-btn'); },
    get travelDateInput() { return document.getElementById('travelDateInput'); },
    get returnDateInput() { return document.getElementById('returnDateInput'); },
    get returnDateGroup() { return document.getElementById('returnDateGroup'); },
 ...
 */
function displayTickets(tickets = [], options = {}) {
    if (!grid) return;
    const { defaultView = false } = options;
    ...
    tickets.forEach(ticket => {
        const ticketElement = createTicketElement(ticket, { defaultView });
...
/**
 * Build default cards: one ticket per destination
 * (used for initial page view to avoid duplicate destinations by class)
 * @param {Array} tickets
 * @returns {Array}
 */
function getDefaultTicketsByDestination(tickets = []) {
    ...
    tickets.forEach((ticket) => {
        ...
        const destinationKey = normalizeArabicText((ticket.direction || '').trim()) || `unknown-${uniqueByDestination.size}`;
        if (!uniqueByDestination.has(destinationKey)) {
            uniqueByDestination.set(destinationKey, ticket);
        }
    });
 ...
 */
function createTicketElement(ticket, options = {}) {
    const { defaultView = false } = options;
    const ticketElement = document.createElement('div');
    ticketElement.className = 'ticket';
    if (defaultView) {
        ticketElement.classList.add('default-ticket-view');
    }
    ...
    const selectedDateForCard = ticket.selectedTripDate || selectedTripDate;
    const selectedReturnDateForCard = ticket.selectedReturnTripDate || selectedReturnTripDate;
    const dateLine = selectedDateForCard
        ? `
                <div class="detail detail-trip-date">
                    <span class="label">تاريخ الرحلة:</span>
                    <span class="value trip-date-value">${escapeHtml(formatDisplayDate(selectedDateForCard))}</span>
                </div>`
        : '';
    const returnDateLine = selectedReturnDateForCard
        ? `
                <div class="detail detail-trip-date">
                    <span class="label">تاريخ العودة:</span>
                    <span class="value trip-date-value">${escapeHtml(formatDisplayDate(selectedReturnDateForCard))}</span>
                </div>`
        : '';

    const normalizedTripMode = tripMode === 'round-trip' ? 'round-trip' : 'one-way';
    const selectedPrice = normalizedTripMode === 'round-trip'
        ? (ticket.priceback || '---')
        : (ticket.pricego || '---');
    const selectedTypeLabel = normalizedTripMode === 'round-trip' ? 'ذهاب وعودة' : 'ذهاب فقط';
    ...
    ticketElement.innerHTML = `
        ...
        <div class="ticket-details">
            ...
            <div class="prices">
                <div class="price-option ${normalizedTripMode === 'round-trip' ? 'highlight' : ''}">
                    ${defaultView ? '' : `<span class="price">$${escapeHtml(selectedPrice)}</span>`}
                    <span class="type">${defaultView ? 'رحلة متاحة - احجز الآن' : selectedTypeLabel}</span>
                    <button class="book-btn ${normalizedTripMode === 'round-trip' ? 'primary' : ''}" onclick="openBookingModal(${JSON.stringify(ticket).replace(/"/g, '&quot;')}, '${selectedTypeLabel}', ${defaultView})">احجز الآن</button>
                </div>
            </div>
        </div>`;
...
function filterTickets() {
    ...
    const travelDate = (document.getElementById('travelDateInput')?.value || '').trim();
    const returnDate = (document.getElementById('returnDateInput')?.value || '').trim();

    if (!travelDate) {
        showSearchNotification('يرجى اختيار تاريخ الرحلة أولاً', 'warning');
        return;
    }
    if (tripMode === 'round-trip' && !returnDate) {
        showSearchNotification('يرجى اختيار تاريخ العودة أولاً', 'warning');
        return;
    }
    ...
    const filteredTickets = allTickets.filter(ticket => {
        ...
        // Airline comes from a select list, so this must be exact.
        const matchesAirline = !airline || 
            normalizeArabicText(airline) === normalizeArabicText(ticket.arlinename || '');
        const matchesDate = isTicketAvailableOnDate(ticket, travelDate);
        const matchesReturnDate = tripMode === 'round-trip'
            ? isTicketAvailableOnDate(ticket, returnDate, true)
            : true;
        const matchesClass = matchesClassFilter(ticket);
        
        return matchesDeparture && matchesDestination && matchesAirline && matchesDate && matchesReturnDate && matchesClass;
    });

    filteredTickets.forEach((ticket) => {
        ticket.selectedTripDate = travelDate;
        ticket.selectedReturnTripDate = tripMode === 'round-trip' ? returnDate : '';
    });

    // Display tickets
    displayTickets(filteredTickets, { defaultView: false });
...
function setupClassFilterButtons() {
    const buttons = DOM.classFilterButtons;
    if (!buttons || !buttons.length) return;

    buttons.forEach((btn) => {
        btn.addEventListener('click', () => {
            const filterValue = btn.getAttribute('data-class-filter') || 'economy';
            selectedClassFilter = filterValue;
            updateActiveClassFilterButton();
            filterTickets();
        });
    });

    selectedClassFilter = 'economy';
    updateActiveClassFilterButton();
}

function updateActiveClassFilterButton() {
    const buttons = DOM.classFilterButtons;
    if (!buttons || !buttons.length) return;
    buttons.forEach((btn) => {
        const isActive = (btn.getAttribute('data-class-filter') || '') === selectedClassFilter;
        btn.classList.toggle('active', isActive);
    });
}

function showClassFilterBar() {
    const classFilterBar = DOM.classFilterBar;
    if (!classFilterBar) return;
    classFilterBar.classList.add('visible');
}

function hideClassFilterBar() {
    const classFilterBar = DOM.classFilterBar;
    if (!classFilterBar) return;
    classFilterBar.classList.remove('visible');
}

function matchesClassFilter(ticket) {
    const ticketClass = getTicketClassColor(ticket?.daraga || '');
    return ticketClass === selectedClassFilter;
}
...
async function loadTickets() {
    ...
    try {
        ...
        displayTickets(getDefaultTicketsByDestination(allTickets), { defaultView: true });
    } catch (error) {
...
function addDaysToIsoDateString(isoDate, days) {
    if (!isValidIsoDateString(isoDate) || typeof days !== 'number') return '';
    const [y, m, d] = isoDate.trim().split('-').map((n) => Number(n));
...
function getTicketDates(ticket) {
    let rawDates = [];

    if (Array.isArray(ticket.tripDates)) {
        rawDates = ticket.tripDates;
    } else if (typeof ticket.tripDates === 'string' && ticket.tripDates.trim()) {
        try {
            const parsed = JSON.parse(ticket.tripDates);
            if (Array.isArray(parsed)) {
                rawDates = parsed;
            }
        } catch (error) {
            // If tripDates is plain string date, keep fallback below.
        }
    }

    const normalizedTripDates = rawDates
        .map((d) => (typeof d === 'string' ? d.trim() : ''))
        .filter((d) => isValidIsoDateString(d));

    if (normalizedTripDates.length > 0) {
        return [...new Set(normalizedTripDates)];
    }

    if (isValidIsoDateString(ticket.date || '')) {
        return [ticket.date.trim()];
    }

    return [];
}

function getTicketReturnDates(ticket) {
    let rawDates = [];

    if (Array.isArray(ticket.returnTripDates)) {
        rawDates = ticket.returnTripDates;
    } else if (typeof ticket.returnTripDates === 'string' && ticket.returnTripDates.trim()) {
        try {
            const parsed = JSON.parse(ticket.returnTripDates);
            if (Array.isArray(parsed)) {
                rawDates = parsed;
            }
        } catch (error) {
            // If returnTripDates is plain string date, keep fallback below.
        }
    }

    const normalizedReturnDates = rawDates
        .map((d) => (typeof d === 'string' ? d.trim() : ''))
        .filter((d) => isValidIsoDateString(d));

    if (normalizedReturnDates.length > 0) {
        return [...new Set(normalizedReturnDates)];
    }

    if (isValidIsoDateString(ticket.returnDate || '')) {
        return [ticket.returnDate.trim()];
    }

    // If the ticket doesn't explicitly define return dates, treat it as one-way only.
    return [];
}

function isTicketAvailableOnDate(ticket, selectedDate, isReturn = false) {
    if (!selectedDate) return true;
    const ticketDates = isReturn ? getTicketReturnDates(ticket) : getTicketDates(ticket);
    return ticketDates.includes(selectedDate);
}

function getAvailableDatesForCriteria(departure, destination, airline) {
    const dates = new Set();
    allTickets.forEach((ticket) => {
        const matchesDeparture =
            normalizeArabicText(departure) === normalizeArabicText(ticket.namecity || '') ||
            normalizeArabicText(departure) === normalizeArabicText(ticket.citycodename || '');
        const matchesDestination =
            normalizeArabicText(destination) === normalizeArabicText(ticket.direction || '') ||
            normalizeArabicText(destination) === normalizeArabicText(ticket.directioncode || '');
        const matchesAirline = !airline ||
            normalizeArabicText(airline) === normalizeArabicText(ticket.arlinename || '');
        if (!matchesDeparture || !matchesDestination || !matchesAirline) return;
        getTicketDates(ticket).forEach((date) => dates.add(date));
    });
    return Array.from(dates).sort();
}

function getAvailableReturnDatesForCriteria(departure, destination, airline, goDate) {
    const dates = new Set();
    allTickets.forEach((ticket) => {
        const matchesDeparture =
            normalizeArabicText(departure) === normalizeArabicText(ticket.namecity || '') ||
        ...
        const matchesAirline = !airline ||
            normalizeArabicText(airline) === normalizeArabicText(ticket.arlinename || '');
        const matchesGoDate = !goDate || getTicketDates(ticket).includes(goDate);
        if (!matchesDeparture || !matchesDestination || !matchesAirline || !matchesGoDate) return;
        getTicketReturnDates(ticket).forEach((date) => dates.add(date));
    });
    return Array.from(dates).sort();
}

function initializeDatePicker() {
    const dateInput = DOM.travelDateInput;
    ...
    datePickerInstance = flatpickr(dateInput, {
        ...
        onOpen: function(selectedDates, dateStr, instance) {
            const dep = (document.getElementById('locationInputnow')?.value || '').trim();
            const dest = (document.getElementById('destinationInput')?.value || '').trim();
            if (!dep || !dest) {
                showSearchNotification('اختر موقع المطار والوجهة أولاً لتفعيل التقويم', 'warning');
                instance.close();
            }
        },
        ...
        onDayCreate: function(dObj, dStr, fp, dayElem) {
            ...
            if (availableDatesSet.has(dayDate)) {
...
function initializeReturnDatePicker() {
    const returnDateInput = DOM.returnDateInput;
    if (!returnDateInput || typeof flatpickr === 'undefined') return;
    returnDateInput.disabled = true;
    returnDatePickerInstance = flatpickr(returnDateInput, {
        dateFormat: 'Y-m-d',
        minDate: 'today',
        allowInput: false,
        clickOpens: true,
        ...
        onOpen: function(selectedDates, dateStr, instance) {
            ...
            const goDate = (document.getElementById('travelDateInput')?.value || '').trim();
            if (!goDate) {
                showSearchNotification('اختر تاريخ الذهاب أولاً لتفعيل تقويم العودة', 'warning');
                instance.close();
            }
        },
        onChange: function(selectedDates, dateStr) {
            selectedReturnTripDate = dateStr || '';
        },
        onDayCreate: function(dObj, dStr, fp, dayElem) {
            const dayDate = fp.formatDate(dayElem.dateObj, 'Y-m-d');
            dayElem.classList.remove('available-day');
            if (availableReturnDatesSet.has(dayDate)) {
                dayElem.classList.add('available-day');
            }
        }
    });
...
function updateDatePickerAvailability() {
    ...
    const airline = (document.getElementById('airlineInput')?.value || '').trim();
    const goDate = (document.getElementById('travelDateInput')?.value || '').trim();
    const hasCoreInputs = departure && destination;
    const availableDates = hasCoreInputs
        ? getAvailableDatesForCriteria(departure, destination, airline)
        : [];
    ...
    if (dateInput) {
        if (!hasCoreInputs) {
            dateInput.placeholder = 'اختر تاريخ الرحلة';
            dateInput.value = '';
            selectedTripDate = '';
            dateInput.disabled = true;
        } else if (availableDates.length === 0) {
            dateInput.placeholder = 'لا توجد تواريخ متاحة لهذا المسار';
            dateInput.value = '';
            selectedTripDate = '';
            dateInput.disabled = true;
        } else if (!selectedTripDate || !availableDatesSet.has(selectedTripDate)) {
            dateInput.placeholder = 'اختر تاريخ الرحلة (الأيام الخضراء متاحة)';
            dateInput.value = '';
            selectedTripDate = '';
    ...
    if (!hasCoreInputs || availableDates.length === 0) {
        datePickerInstance.set('enable', []);
        datePickerInstance.set('disable', [() => true]);
    } else {
        datePickerInstance.set('enable', availableDates);
        datePickerInstance.set('disable', [date => !availableDatesSet.has(datePickerInstance.formatDate(date, 'Y-m-d'))]);
    }
    datePickerInstance.redraw();
    updateReturnDatePickerAvailability(goDate);
}

function updateReturnDatePickerAvailability(goDate = '') {
    if (!returnDatePickerInstance) return;
    const departure = (document.getElementById('locationInputnow')?.value || '').trim();
    const destination = (document.getElementById('destinationInput')?.value || '').trim();
    const airline = (document.getElementById('airlineInput')?.value || '').trim();
    const hasCoreInputs = departure && destination;
    const rawReturnDates = (tripMode === 'round-trip' && hasCoreInputs && goDate)
        ? getAvailableReturnDatesForCriteria(departure, destination, airline, goDate)
        : [];

    const normalizedGoDate = isValidIsoDateString(goDate) ? goDate.trim() : '';
    const minReturnDate = normalizedGoDate ? addDaysToIsoDateString(normalizedGoDate, 1) : '';
    const availableReturnDates = minReturnDate
        ? rawReturnDates.filter((d) => isValidIsoDateString(d) && d >= minReturnDate)
        : rawReturnDates;

    availableReturnDatesSet = new Set(availableReturnDates);
    ...
    if (tripMode !== 'round-trip') {
        ...
        selectedReturnTripDate = '';
        returnDatePickerInstance.set('minDate', 'today');
        returnDatePickerInstance.set('enable', []);
        returnDatePickerInstance.set('disable', [() => true]);
    } else if (!goDate) {
        returnDateInput.disabled = true;
        returnDateInput.placeholder = 'اختر تاريخ الذهاب أولاً';
        returnDateInput.value = '';
        selectedReturnTripDate = '';
        returnDatePickerInstance.set('minDate', 'today');
        returnDatePickerInstance.set('enable', []);
        returnDatePickerInstance.set('disable', [() => true]);
    } else if (!availableReturnDates.length) {
        returnDateInput.disabled = true;
        returnDateInput.placeholder = 'لا توجد تواريخ عودة متاحة';
        returnDateInput.value = '';
        selectedReturnTripDate = '';
        returnDatePickerInstance.set('minDate', minReturnDate || 'today');
        returnDatePickerInstance.set('enable', []);
        returnDatePickerInstance.set('disable', [() => true]);
    } else {
        returnDateInput.disabled = false;
        returnDateInput.placeholder = 'اختر تاريخ العودة (الأيام الخضراء متاحة)';
        returnDatePickerInstance.set('minDate', minReturnDate || 'today');

        if (!selectedReturnTripDate || !availableReturnDatesSet.has(selectedReturnTripDate)) {
            returnDateInput.value = '';
            selectedReturnTripDate = '';
        }
        returnDatePickerInstance.set('enable', availableReturnDates);
        returnDatePickerInstance.set('disable', [date => !availableReturnDatesSet.has(returnDatePickerInstance.formatDate(date, 'Y-m-d'))]);
    }
    returnDatePickerInstance.redraw();
}

function setupTripModeSwitch() {
    const modeButtons = document.querySelectorAll('.trip-mode-btn');
    modeButtons.forEach((btn) => {
        btn.addEventListener('click', () => {
            modeButtons.forEach((b) => b.classList.remove('active'));
            btn.classList.add('active');
            tripMode = btn.getAttribute('data-trip-mode') || 'one-way';
            if (DOM.returnDateGroup) {
                DOM.returnDateGroup.style.display = tripMode === 'round-trip' ? 'block' : 'none';
            }
            resetSearchBarsOnTripModeChange();
            selectedReturnTripDate = '';
...
function resetSearchBarsOnTripModeChange() {
    const departureInput = document.getElementById('locationInputnow');
    const destinationInput = document.getElementById('destinationInput');
    ...
    // Reset airline select/input to default
    if (airlineInput) {
        airlineInput.value = '';
    }
    ...
    // Clear date pickers internal selected values as well
    if (datePickerInstance) {
        datePickerInstance.clear();
    }
    if (returnDatePickerInstance) {
        returnDatePickerInstance.clear();
    }

    // Hide & clear autocomplete suggestions immediately
    document.querySelectorAll('.suggestions').forEach((container) => {
        container.style.display = 'none';
        container.innerHTML = '';
    });
    if (autocompleteSystem && typeof autocompleteSystem.hideAllSuggestions === 'function') {
        autocompleteSystem.hideAllSuggestions();
    }

    // Reset visual/input state for search containers
    document.querySelectorAll('.search-input-container').forEach((container) => {
        container.classList.remove('active', 'error', 'success', 'has-value');
    });
    document.querySelectorAll('.search-input-container .search-input').forEach((input) => {
        if (typeof input.blur === 'function') input.blur();
    });

    // Reset results to default list (prevents mismatch with cleared filters)
    if (grid && Array.isArray(allTickets) && allTickets.length) {
        // Remove per-search selected dates stored on ticket objects
        allTickets.forEach((ticket) => {
            if (!ticket || typeof ticket !== 'object') return;
            ticket.selectedTripDate = '';
            ticket.selectedReturnTripDate = '';
        });
        displayTickets(getDefaultTicketsByDestination(allTickets), { defaultView: true });
    }
}

function setupDateDependencies() {
    const departureInput = document.getElementById('locationInputnow');
    const destinationInput = document.getElementById('destinationInput');
    const airlineInput = document.getElementById('airlineInput');
    [departureInput, destinationInput].forEach((input) => {
        if (!input) return;
        input.addEventListener('change', updateDatePickerAvailability);
        input.addEventListener('input', updateDatePickerAvailability);
    });
    if (airlineInput) {
        airlineInput.addEventListener('change', updateDatePickerAvailability);
    }
    const goDateInput = document.getElementById('travelDateInput');
    if (goDateInput) {
        goDateInput.addEventListener('change', () => {
            updateReturnDatePickerAvailability(goDateInput.value.trim());
        });
    }
}
...
class AutocompleteSystem {
    ...
    async init() {
        await this.loadData();
        await this.loadAirlinesFromAdminFile();
        this.populateAirlineDropdown();
    ...
    async loadData() {
        try {
            ...
            tickets.forEach(ticket => {
                ...
                if (ticket.namecity) {
                    departureCitiesSet.add(ticket.namecity.trim());
                }
                
                // Add destination cities only for destination input
                if (ticket.direction) {
                    destinationCitiesSet.add(ticket.direction.trim());
                }
            ...
            this.departureCities = Array.from(departureCitiesSet).sort();
            this.destinationCities = Array.from(destinationCitiesSet).sort();
            this.cities = [...this.departureCities, ...this.destinationCities];
    ...
    /**
     * Load airlines from admin-panel/airlines.json
     */
    async loadAirlinesFromAdminFile() {
        try {
            const response = await fetch('admin-panel/airlines.json');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            ...
            if (Array.isArray(data.airlines)) {
                const airlinesFromAdmin = data.airlines
                    .map(item => (item && item.name ? String(item.name).trim() : ''))
                    .filter(Boolean);

                if (airlinesFromAdmin.length > 0) {
                    this.airlines = [...new Set(airlinesFromAdmin)].sort();
                }
            }
        } catch (error) {
            console.warn('Could not load admin airlines list, using data.json airlines as fallback.', error);
        }
    }
    ...
    setupEventListeners() {
        ...
        const airlineInput = document.getElementById('airlineInput');
        if (airlineInput) {
            if (airlineInput.tagName === 'SELECT') {
                airlineInput.addEventListener('change', () => {
                    if (searchButton) {
                        filterTickets();
                    }
                });
    ...
    /**
     * Populate airline select dropdown from loaded airlines
     */
    populateAirlineDropdown() {
        const airlineInput = document.getElementById('airlineInput');
        if (!airlineInput || airlineInput.tagName !== 'SELECT') return;

        airlineInput.innerHTML = '<option value="">كل شركات الطيران</option>';
    ...
    handleInput(input, type, suggestionsContainer) {
        ...
        if (type === 'airline') {
            ...
            suggestions = this.departureCities.filter(item => 
                arabicTextMatch(query, item)
            );
        } else {
            // Show only destination cities for destination input
            suggestions = this.destinationCities.filter(item => 
                arabicTextMatch(query, item)
            );
        }
        ...
        container.innerHTML = limitedSuggestions.map((suggestion, index) => {
            ...
            return `
                <div class="suggestion-item" data-index="${index}" data-value="${displayName}">
                    <i class="fas ${type === 'airline' ? 'fa-plane' : type === 'destination' ? 'fa-plane-arrival' : 'fa-map-marker-alt'}"></i>
...
let currentTicketReturnDate = null;
let currentTicketAirline = null;
let currentTicketClass = null;
let currentBookingFromDefaultView = false;

/**
 * Open booking modal
 */
function openBookingModal(ticket, ticketType = null, fromDefaultView = false) {
    const modal = document.getElementById('bookingModal');
    if (modal) {
        currentBookingFromDefaultView = fromDefaultView === true;
        // Store ticket destination if provided
        if (ticket) {
            currentTicketDestination = ticket.direction || 'الرحلات المتاحة';
            currentTicketDate = ticket.selectedTripDate || selectedTripDate || null;
            currentTicketReturnDate = ticket.selectedReturnTripDate || selectedReturnTripDate || null;
            currentTicketAirline = ticket.arlinename || null;
            currentTicketClass = ticket.daraga || null;
...
function closeBookingModal() {
    ...
    if (modal) {
        ...
        currentTicketType = null;
        currentTicketDate = null;
        currentTicketReturnDate = null;
        currentTicketAirline = null;
        currentTicketClass = null;
        currentBookingFromDefaultView = false;
    }
}
...
function openWhatsApp() {
    ...
    if (!currentBookingFromDefaultView) {
        if (currentTicketType) {
            message += ` (${currentTicketType})`;
        }
        if (currentTicketDate) {
            message += ` بتاريخ ${formatDisplayDate(currentTicketDate)}`;
        }
        if (currentTicketReturnDate) {
            message += ` والعودة بتاريخ ${formatDisplayDate(currentTicketReturnDate)}`;
        }
        if (currentTicketAirline) {
            message += ` | شركة الطيران: ${currentTicketAirline}`;
        }
        if (currentTicketClass) {
            message += ` | درجة التذكرة: ${currentTicketClass}`;
        }
    }
    message += ` ✈️`;
...
class ProfessionalCarousel {
    ...
    /**
     * Return a random slide index different from the current index
     */
    getRandomIndexExceptCurrent() {
        if (this.state.totalSlides <= 1) return 0;

        let randomIndex = Math.floor(Math.random() * this.state.totalSlides);
        while (randomIndex === this.state.currentIndex) {
            randomIndex = Math.floor(Math.random() * this.state.totalSlides);
        }
        return randomIndex;
    }
    ...
    async loadData() {
        ...
        try {
            ...
            const data = await response.json();
            const tickets = Array.isArray(data.data) ? data.data : [];
            this.state.data = this.shuffleTickets(tickets);
            this.state.totalSlides = this.state.data.length;
            if (this.state.totalSlides > 1) {
                this.state.currentIndex = Math.floor(Math.random() * this.state.totalSlides);
            }
    ...
    /**
     * Shuffle tickets so carousel order is random each load
     */
    shuffleTickets(tickets) {
        ...
        for (let i = shuffledTickets.length - 1; i > 0; i--) {
            ...
            [shuffledTickets[i], shuffledTickets[randomIndex]] = [shuffledTickets[randomIndex], shuffledTickets[i]];
        }
    ...
    startAutoPlay() {
        ...
        this.autoPlayTimer = setInterval(() => {
            if (!this.state.isPaused && !this.state.isAnimating) {
                this.state.currentIndex = this.getRandomIndexExceptCurrent();
                this.animateTransition();
            }
